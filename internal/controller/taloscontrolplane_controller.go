...
func (r *TalosControlPlaneReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	ready := meta.IsStatusConditionTrue(tcp.Status.Conditions, talosv1alpha1.ConditionReady)
	status := "not_ready"
	if ready {
		status = "ready"
	}
	operatormetrics.SetResourceStatus("taloscontrolplane", tcp.Namespace, tcp.Name, status, 1.0)
